...
async function carregar_usuarios(valor) {
    if (valor.length >= 3) {
        ...
        const dados = await fetch(getRoot()+'controllers/controllerPesquisa.php?title=' + valor);
        const resposta = await dados.json();
        var nome = 'New value';
        var id_usuario = 'New value';
...
const fechar = document.getElementById("title");

$('resultado_pesquisa').on('focus',function(){
